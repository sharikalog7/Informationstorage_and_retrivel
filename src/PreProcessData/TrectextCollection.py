...
class TrectextCollection:
    ...
    def __init__(self):
        ...
        f = open(Path.DataTextDir, errors="ignore")
        xml = f.read()
        self.m = re.compile('<DOC>(.*?)</DOC>', re.DOTALL).findall(xml) # Fetches all data between each <Doc></Doc> combinations and stores it as a list
        return
    ...
    def nextDocument(self):
        ...
        if(self.doc_no >= len(self.m)): # Iterate till last document
            return None
